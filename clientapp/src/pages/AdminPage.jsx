...
const AdminPage = () => {
  ...
  const fetchPendingStories = async () => {
    try {
      ...
      console.error('Lỗi khi lấy danh sách truyện chờ duyệt:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchPendingChapters = async () => {
    try {
      ...
      console.error('Lỗi khi lấy danh sách chương chờ duyệt:', error);
    } finally {
      setLoading(false);
    }
  };
                    ...
                    {pendingChapters.map(chapter => (
                        chapter.approved === false && (
                            <div key={chapter._id} className="border border-gray-200 p-4 rounded mb-4">
                                ...
                                <Text className="text-gray-700 block mb-2">Truyện: {chapter.story.title}</Text> {/* Hiển thị tên truyện */}
